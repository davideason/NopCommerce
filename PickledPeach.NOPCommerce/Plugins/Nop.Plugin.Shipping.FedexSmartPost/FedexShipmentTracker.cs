...
namespace Nop.Plugin.Shipping.FedexSmartPost
{
    public class FedexShipmentTracker : IShipmentTracker
    {
        ...
        private readonly FedexSmartPostSettings _fedexSmartPostSettings;

        public FedexShipmentTracker(ILogger logger, FedexSmartPostSettings fedexSPSettings)
        {
            this._logger = logger;
            this._fedexSmartPostSettings = fedexSPSettings;
        }
        ...
        public virtual IList<ShipmentStatusEvent> GetShipmentEvents(string trackingNumber)
        {
            ...
            try
            {
                ...
                request.WebAuthenticationDetail.UserCredential.Key = _fedexSmartPostSettings.Key; // Replace "XXX" with the Key
                request.WebAuthenticationDetail.UserCredential.Password = _fedexSmartPostSettings.Password; // Replace "XXX" with the Password
                //
                request.ClientDetail = new ClientDetail();
                request.ClientDetail.AccountNumber = _fedexSmartPostSettings.AccountNumber; // Replace "XXX" with client's account number
                request.ClientDetail.MeterNumber = _fedexSmartPostSettings.MeterNumber; // Replace "XXX" with client's meter number
                //
                ...
                var service = new TrackService(_fedexSmartPostSettings.Url);
            ...
            catch (SoapException ex)
            {
                ...
                _logger.Error(string.Format("Error while getting FedexSmartPost shipment tracking info - {0}", trackingNumber), new Exception(sb.ToString()));
            }
            catch (Exception exc)
            {
                _logger.Error(string.Format("Error while getting FedexSmartPost shipment tracking info - {0}", trackingNumber), exc);
            }
            return result;
        }
    }
